...
func TestSearchTitlesAcceptsNumericPicField(t *testing.T) {
	...
	if titles[0].Type != "series" {
		t.Fatalf("expected first result to be series, got %q", titles[0].Type)
	}

	if titles[1].IMDBID != 0 {
		t.Fatalf("expected numeric pic=0 to map to imdb id 0, got %d", titles[1].IMDBID)
	}
}
